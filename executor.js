...
class Interpreter extends BaseCstVisitor {
	...
	comparison(ctx) {
		...
		if (ctx.Identifier) {
			if (!(ctx.Identifier[0].image in identifiers)) throw "Identifier needs to be initialized before comparison"
	...
	mathExpr(ctx) {
		...
		if (ctx.Identifier) {
			if (!(ctx.Identifier[0].image in identifiers)) throw "Identifier needs to be initialized before computation"
